...
// CreateClusterReq This is a auto create Body Object
type CreateClusterReq struct {
	Cluster CreateClusterBody `json:"cluster" required:"true"`
}

// CreateClusterBody
type CreateClusterBody struct {
	// Instance. For details about related parameters, see Table 4.
	Instance *CreateClusterInstanceBody `json:"instance" required:"true"`
	// Type of the data search engine. For details about related parameters, see Table 7.
	Datastore *CreateClusterDatastoreBody `json:"datastore,omitempty"`
	...
	// Automatic snapshot creation. This function is enabled by default. For details about related parameters, see Table 8.
	...
	DiskEncryption *CreateClusterEncryptionBody `json:"diskEncryption,omitempty"`
...
type CreateClusterInstanceBody struct {
	...
	Nics CreateClusterInstanceNicsBody `json:"nics" required:"true"`
...
// CreateClusterInstanceVolumeBody volume
...
type CreateClusterEncryptionBody struct {
...
func CreateCluster(c *golangsdk.ServiceClient, opts CreateClusterReq) (*CreateClusterResponse, error) {
	...
	_, err = c.Post(createClusterURL(c), b, &r, &golangsdk.RequestOpts{
...
func QueryingClusterDetail(c *golangsdk.ServiceClient, clusterId string) (*ClusterDetailResponse, error) {
	var rst ClusterDetailResponse
	_, err := c.Get(queryingClusterDetailURL(c, clusterId), &rst, &golangsdk.RequestOpts{
		MoreHeaders: RequestOpts.MoreHeaders,
	})
...
func DeleteCluster(c *golangsdk.ServiceClient, clusterId string) *golangsdk.ErrResult {
	var r golangsdk.ErrResult
	_, r.Err = c.Delete(deleteClusterURL(c, clusterId), &golangsdk.RequestOpts{
		MoreHeaders: RequestOpts.MoreHeaders,
	})
	return &r
}
...
// ListClustersDetails
func ListClustersDetails(c *golangsdk.ServiceClient, opts ListClustersDetailsOpts) (*ClusterListResponse, error) {
	url := listClustersDetailsURL(c)
...
// RestartCluster
func RestartCluster(c *golangsdk.ServiceClient, clusterId string) (*RestartClusterResponse, error) {

	var rst RestartClusterResponse
	_, err := c.Post(restartClusterURL(c, clusterId), nil, &rst, &golangsdk.RequestOpts{
		MoreHeaders: RequestOpts.MoreHeaders,
	})
	if err == nil {
		return &rst, nil
